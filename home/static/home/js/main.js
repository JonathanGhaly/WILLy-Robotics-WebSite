...
function is_mobile_device() {
  if (
    $(window).width() < 767 ||
    navigator.userAgent.match(/(Android|iPhone|iPod|iPad)/)
  ) {
    return true;
  } else {
    return false;
...
function initialize() {
  var stylez = [
    ...
    },
    {
      ...
      elementType: "geometry",
      stylers: [{ color: "#f88f6f" }],
    },
    {
      featureType: "road.highway",
      elementType: "labels.text.fill",
      stylers: [{ visibility: "on" }, { color: "#8b3f38" }],
    },
    {
      featureType: "road.highway",
    ...
    },
    {
      ...
      stylers: [{ color: "#eee4e0" }, { lightness: -10 }],
    },
    {
      featureType: "landscape",
    ...
    },
    {
      ...
      elementType: "geometry",
      stylers: [{ visibility: "simplified" }, { color: "#d2d2d2" }],
    },
    {},
    {
      featureType: "road.local",
      elementType: "labels.text.fill",
      stylers: [{ color: "#040c06" }],
    },
    {
      featureType: "transit.station",
      elementType: "labels.text.fill",
      stylers: [{ visibility: "on" }, { color: "#1d080f" }],
    },
    {
      featureType: "water",
      stylers: [{ color: "#cbdbf0" }, { hue: "#005eff" }, { saturation: 41 }],
    },
    {
      featureType: "administrative",
      elementType: "labels.text.fill",
      stylers: [{ color: "#7f5e51" }],
    },
    {
      featureType: "poi",
      elementType: "geometry.fill",
      stylers: [{ color: "#fdeee0" }],
    },
    {
      featureType: "poi.park",
      elementType: "geometry.fill",
      stylers: [{ color: "#e6f9c1" }],
    },
    {
      featureType: "road.highway.controlled_access",
      elementType: "labels.icon",
      stylers: [{ hue: "#627dbc" }, { saturation: -82 }, { gamma: 0.32 }],
    },
    {
      featureType: "transit.station",
      elementType: "labels.icon",
...
jQuery.fn.parallax = function () {
  ...
  this.each(function () {
    ...
    var corectir =
      (imgHeight - bgContHeight - (imgHeight - bgContHeight) * scrollCoef) *
      topCoef;
    ...
    if (winWidth > 1024) {
      ...
      if (imgWidth != 0) {
        ...
        parallaxContainer.css({
          "-webkit-transform": "translateY(" + parOfs + "px) translateZ(0)",
          "-moz-transform": "translateY(" + parOfs + "px) translateZ(0)",
          "-ms-transform": "translateY(" + parOfs + "px) translateZ(0)",
          transform: "translateY(" + parOfs + "px) translateZ(0)",
          left: +leftOfs + "px",
        });
      }
      ...
      parallaxContainer.css({
        ...
        "-webkit-transform":
          "translateY(" +
          parseInt((img.height() - bgobj.outerHeight()) / -2, 10) +
          "px) translateZ(0)",
        "-moz-transform":
          "translateY(" +
          parseInt((img.height() - bgobj.outerHeight()) / -2, 10) +
      ...
      if (img.height() < parallaxContainer.height()) {
        ...
        parallaxContainer.css({
          left:
            +parseInt((img.width() - parallaxContainer.width()) / -2, 10) +
            "px",
...
$(document).ready(function () {
    ...
    $(function () {
      var event_content = {
        0: {
          ...
          container: "day-view-wrap",
          carousel: "carousel-list-view.html",
        },
        1: {
          view: "week-view",
          container: "week-view-wrap",
          carousel: "carousel-week-view.html",
        },
        2: {
          view: "month",
          container: "event-calendar",
          carousel: "carousel-month-view.html",
        },
      };
      var i = 2;
      ...
      function qwer() {
        $.ajax({
          ...
          success: function (html) {
            ...
            $(".event-container").height(
              $(".event-container>div:last-child").outerHeight(true)
            );
        ...
        $.ajax({
          ...
          error: function () {
            $(".calendar-header .buttons-carousel .carousel-list").remove();
            $(".calendar-header .buttons-carousel").append(
              "<div class='carousel-list'><i class='fa fa-calendar'></i> July 2015</div>"
            );
          },
        });
      }
  ...
  $(document).on(
    "mouseenter",
    ".event-calendar td, .week-view-wrap td",
    function () {
  ...
  /* widget menu */
  /**/
  $(".widget-navigation li>ul, .widget-pages li>ul")
    .parent()
    .addClass("has-child");
  ...
  if ($("ul#flickr-badge").length) {
    jQuery("ul#flickr-badge").jflickrfeed({
      ...
      itemTemplate:
        '<li><div class="flickr-container"><a href="http://www.flickr.com/photos/56342020@N03"><span><img src="{{image_m}}" alt="{{title}}" /></span></a></div></li>',
    });
  }
  ...
  $(".twitter-carousel").tweet({
    ...
    template:
      "<i class='fa fa-twitter twitt'></i><p><a class='cp-color' href='{user_url}'>@{screen_name}</a>{join}{text}<br>{time}</p>",
  });
  ...
  $(".twitter-carousel .tweet_list")
    .addClass("widget-carousel")
  ...
  $(".twitter-1").tweet({
    ...
    loading_text: "loading twitter feed...",
    template:
      "<i class='fa fa-twitter twitt'></i><div class='divider-mini'></div><p><a class='cp-color' href='{user_url}'>@{screen_name}</a>{join}{text}<br>{time}</p>",
  });

  /**/
  /* wrap tweet list for 2 elements */
  /**/
  $(".twitter-1 .tweet_list")
    .addClass("testimonials-carousel")
    .addClass("owl-carousel");
...
function mobile_nav_switcher_init() {
  ...
  jQuery(document).on(
    "click",
    "nav.main-nav.mobile_nav .mobile_menu_switcher",
    function () {
    ...
    $(this).validate(
      /*feedback-form*/ {
        ...
        errorLabelContainer: $(this)
          .parent()
          .children(".alert-boxes.error-alert")
          .children(".message"),
        rules: {
          name: {
            required: true,
    ...
    $.post("php/contacts-process.php", data, function (s_response) {
      ...
      if (s_response.info == "success") {
        $wrapper
          ...
          .append(
            "<div class='info-boxes confirmation-message' id='feedback-form-success'><strong>Success!</strong><br><p>Your message was successfully delivered.</p></div>"
          );
        ...
        $wrapper
          ...
          .append(
            "<div class='message_box error-box'><div class='message_box_header'>Error Box</div><p>Server fail! Please try again later!</p></div>"
          );
        $wrapper.delay(5000).hide(500, function () {
          $(this).removeClass("message message-success").text("").fadeIn(500);
          $wrapper.css("display", "none");
...
var win = window,
    ...
    if(x<= 425){
    document.getElementById("#footer-text-id").style.width = x;
